 ...
 * @brief Shared Csound synthesis backend.
 *
 * Implements the shared Csound API using the Csound library.
 ...
 *
 * This backend is shared by all languages (Alda, Joy, etc.).
 */
...
int shared_csound_is_available(void) {
    return 1;  /* Csound is compiled in */
}
...
static void csound_audio_callback(ma_device* device, void* output, const void* input, ma_uint32 frame_count) {
    ...
    if (g_cs.enabled && g_cs.started) {
        shared_csound_render(out, (int)frame_count);
    } else {
...
void shared_csound_send_note_on_freq(int channel, double freq, int velocity, int midi_pitch) {
    ...
    /* Velocity 0 is note off */
    if (velocity == 0) {
        shared_csound_send_note_off(channel, midi_pitch);
        return;
    }
...
int shared_csound_is_available(void) { return 0; }
int shared_csound_init(void) { return -1; }
void shared_csound_cleanup(void) {}
int shared_csound_load(const char* path) { (void)path; return -1; }
int shared_csound_compile_orc(const char* orc) { (void)orc; return -1; }
int shared_csound_has_instruments(void) { return 0; }
int shared_csound_enable(void) { return -1; }
void shared_csound_disable(void) {}
int shared_csound_is_enabled(void) { return 0; }
void shared_csound_send_note_on(int ch, int p, int v) { (void)ch; (void)p; (void)v; }
void shared_csound_send_note_on_freq(int ch, double f, int v, int p) { (void)ch; (void)f; (void)v; (void)p; }
void shared_csound_send_note_off(int ch, int p) { (void)ch; (void)p; }
void shared_csound_send_program(int ch, int p) { (void)ch; (void)p; }
void shared_csound_send_cc(int ch, int cc, int v) { (void)ch; (void)cc; (void)v; }
void shared_csound_send_pitch_bend(int ch, int b) { (void)ch; (void)b; }
void shared_csound_all_notes_off(void) {}
...
int shared_csound_playback_active(void) { return 0; }
void shared_csound_stop_playback(void) {}
